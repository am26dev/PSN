...
import { obterUnidadeParaMarcacao } from "@/lib/services/unidade.service";
import { obterMarcacaoParaRemarcar } from "@/lib/services/marcacao.service";
import { listarDependentes } from "@/lib/services/dependente.service";
...
export default async function MarcarPage({
  ...
  const remarcar = sp.remarcar
    ? await obterMarcacaoParaRemarcar(utente.id, id, sp.remarcar)
    : null;

  const unidade = await obterUnidadeParaMarcacao(id);

  if (!unidade || !unidade.ativo || unidade.tipo === "FARMACIA") notFound();

  const dependentes = await listarDependentes(utente.id);

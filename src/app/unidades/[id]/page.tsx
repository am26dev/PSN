...
import {
  obterUnidadeDetalhe,
  listarUnidadesRede,
} from "@/lib/services/unidade.service";
import { marcacaoAtivaDoUtente } from "@/lib/services/marcacao.service";
...
export default async function UnidadePage({
  ...
  const unidade = await obterUnidadeDetalhe(id);
  ...
  const marcacaoAtiva = utente
    ? await marcacaoAtivaDoUtente(utente.id, unidade.id)
    : null;
  ...
  const unidadesRede = unidade.rede
    ? await listarUnidadesRede(unidade.rede, unidade.id)
    : [];

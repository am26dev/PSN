...
import { contarUnidadesCached } from "@/lib/services/unidade.service";
import { listarTodasSeguradoras } from "@/lib/services/seguro.service";

export default async function HomePage() {
  const [hospitais, prestadores, farmacias] = await Promise.all([
    contarUnidadesCached({ tipo: "HOSPITAL_PUBLICO", ativo: true }),
    contarUnidadesCached({ tipo: { notIn: ["HOSPITAL_PUBLICO", "FARMACIA"] }, ativo: true }),
    contarUnidadesCached({ tipo: "FARMACIA", ativo: true }),
  ]).catch(() => [0, 0, 0]);

  const seguradoras = await listarTodasSeguradoras().catch(() => []);
